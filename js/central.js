...
async function initSyncPage() {
    try {
        ...
        // Start central websocket and render helpers
        startCentralHelpersView();
    } catch (error) {
        console.error('Error initializing sync page:', error);
    }
}

// Render helpers list in the Helfer Status table and keep it updated via central websocket
function renderHelpersTable(helpers) {
    ...
    helpers.forEach(h => {
        ...
        const statusIcon = h.isAlive ? '<span class="material-icons-round">cloud_circle</span> Live' : '<span class="material-icons-round">cloud_off</span> Offline';
        const lastSync = h.lastSync ? timeAgo(new Date(h.lastSync)) : 'Nie';
        tr.innerHTML = `
            <td>${h.name}</td>
...
function startCentralHelpersView() {
    api.startCentralWebSocket((helpers) => {
        renderHelpersTable(helpers);
    });
    // request initial clients after short delay
    setInterval(() => api.centralRequestClients(), 1000);
}
